...
class JavaLanguageServer extends AutoLanguageClient {
  ...
  startServerProcess() {
    ...
    if (config == null) {
      throw `${this.getServerName()} not supported on ${process.platform}`;
    }
  ...
  getOrInstallLauncher(serverHome) {
    ...
    if (launchers.length < 1) {
      ...
      if (launchers.length < 1) {
        throw `Failed to install the ${this.getServerName()} language server`;
      }
    }
  ...
  postInitialization(InitializationResult) {
    this._lc.onCustom('language/status', (e) => this.statusElement.textContent = `${this.name} ${e.type.replace(/^Started$/, '')} ${e.message}`);
