...
class JavaLanguageClient extends AutoLanguageClient {
  ...
  installServer (serverHome) {
    ...
    this.setBusyMessage(`Downloading ${this.getServerName()} language support`);
    return this.fileExists(serverHome)
      .then(doesExist => { if (!doesExist) fs.mkdirSync(serverHome) })
      .then(() => DownloadFile(serverDownloadUrl, localFileName, (bytesDone, percent) => {
          this.updateStatusBar(`downloading ${percent}%`)
          this.setBusyMessage(`Downloading <b>${percent}%</b>`)
        }, serverDownloadSize))
      .then(() => {
          this.updateStatusBar('unpacking');
          this.setBusyMessage(`Unpacking ${this.getServerName()} language support`);
        })
