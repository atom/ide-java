...
class AutoImportActionProvider {
      ...
      .map(d => {
        ...
        return {
          ...
          apply: () => {
            ...
            .then(importChoice => {
              console.debug('Import choice:', importChoice)
              this.addImport(editor, importChoice)
  ...
  **/
  addImport(editor, importPkg) {
    console.debug('getLocationForImport called w/', editor)

    let importInsertionRange

    // get first occurrence of import
    const importGroupExpr = /import.+$/ 
    editor.scan(importGroupExpr, { leadingContextLineCount: 1 }, function() {
      console.debug('Scan got args ', arguments)
      const {match, computedRange} = arguments
      console.debug('Match:', match)
      console.debug('Found in range:', computedRange)
      importInsertionRange = computedRange
    })
    console.debug('Scan has exited')

    const packageStmtExpr = /package.+$/
    if (!importInsertionRange) {
      editor.scan(packageStmtExpr, ({ match, computedRange }) => {
        console.debug('Package match:', match)
        importInsertionRange = computedRange
      })
    }
